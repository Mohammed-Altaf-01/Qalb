...
import { isAppUserStorageNamespace, validateAppUserStoragePayload, validateActivityMetadata } from "@/lib/app-user-storage";
...
describe("app-user-storage validation", () => {
  ...
  it("accepts read_key_themes namespace", () => {
    ...
    const r = validateAppUserStoragePayload("read_key_themes", { themesBySurahId: { "1": { markdown: "Hi", updatedAt: 1 } } });
    expect(r.ok).toBe(true);
  });
