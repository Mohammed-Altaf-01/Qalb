...
import { getEntriesForReciter } from "../listen-history.js";
import {
  mergeHifzCards,
...
describe("mergeListenHistoryPayload", () => {
  const entry = (surahId, updatedAt, reciterId = 7) => ({
    reciterId,
    reciterName: "R",
    surahId,
    surahName: `S${surahId}`,
  ...
  it("keeps newer updatedAt per reciter+surah", () => {
    const m = mergeListenHistoryPayload(
      { entries: [entry(2, 100)] },
      { entries: [entry(2, 200, 7)] },
    );
    expect(m.entries).toHaveLength(1);
    expect(m.entries[0].updatedAt).toBe(200);
  });

  it("merges distinct surahs and caps per reciter", () => {
    const remote = { entries: [entry(1, 1), entry(2, 2)] };
    const local = { entries: [entry(3, 3), entry(4, 4), entry(5, 5), entry(6, 6), entry(7, 7)] };
    const m = mergeListenHistoryPayload(remote, local);
    expect(getEntriesForReciter(m.entries, 7)).toHaveLength(5);
  });
});
...
describe("mergeKhatmPayload", () => {
  it("unions pages and milestones", () => {
    const m = mergeKhatmPayload({ pages: [1, 2], milestones: { overall: [30], juz: { 1: [50] } } }, [2, 3], {
      overall: [50],
      juz: { 2: [30] },
    });
    expect(m.pages).toEqual([1, 2, 3]);
    expect(m.milestones.overall).toEqual(expect.arrayContaining([30, 50]));
    expect(m.milestones.juz["1"]).toEqual([50]);
    expect(m.milestones.juz["2"]).toEqual([30]);
  });
});

describe("mergeHifzCards", () => {
  it("keeps card with newer updatedAt", () => {
    const m = mergeHifzCards(
      { cards: { "1:1": { scheduling: {}, updatedAt: 10 } } },
      { "1:1": { scheduling: {}, updatedAt: 50 }, "2:2": { scheduling: {}, updatedAt: 5 } },
    );
    expect(m["1:1"].updatedAt).toBe(50);
    expect(m["2:2"].updatedAt).toBe(5);
  });
});
