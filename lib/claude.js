...
class AIService {
  ...
  async discoverVerses(userSituation, searchResults = []) {
    ...
    const t0 = Date.now();
    aiLog.info("discover_claude_start", {
      situationChars: userSituation?.length ?? 0,
      candidateRows: searchResults.length,
    });
    ...
    aiLog.info("discover_claude_done", { durationMs: Date.now() - t0, stopReason: response?.stop_reason });
  ...
  async generateReflectionPrompts(verseKey, verseText, translation, tafsirSnippet = "") {
    const t0 = Date.now();
    aiLog.info("reflect_claude_start", {
      verseKey,
      translationChars: translation?.length ?? 0,
      tafsirChars: tafsirSnippet?.length ?? 0,
    });
    const response = await this.client.messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 512,
    ...
    aiLog.info("reflect_claude_done", { durationMs: Date.now() - t0 });
