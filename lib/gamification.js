...
export const LEVELS = [
  ...
  { level: 2, icon: "📖", title: "Reader", titleAr: "قارئ", minXp: 60, color: "text-emerald-400", bg: "bg-emerald-400/10" },
  { level: 3, icon: "🧠", title: "Student", titleAr: "طالب علم", minXp: 170, color: "text-sky-400", bg: "bg-sky-400/10" },
  { level: 4, icon: "🎓", title: "Scholar", titleAr: "عالم", minXp: 420, color: "text-violet-400", bg: "bg-violet-400/10" },
  { level: 5, icon: "📿", title: "Hafiz", titleAr: "حافظ", minXp: 900, color: "text-amber-400", bg: "bg-amber-400/10" },
  { level: 6, icon: "🕌", title: "Imam", titleAr: "إمام", minXp: 1800, color: "text-yellow-400", bg: "bg-yellow-400/10" },
  { level: 7, icon: "✨", title: "Murabbi", titleAr: "مربّي", minXp: 3300, color: "text-fuchsia-400", bg: "bg-fuchsia-400/10" },
  { level: 8, icon: "🌙", title: "Mujahid", titleAr: "مجاهد", minXp: 5600, color: "text-indigo-400", bg: "bg-indigo-400/10" },
  { level: 9, icon: "🕯️", title: "Muhsin", titleAr: "محسن", minXp: 9000, color: "text-rose-300", bg: "bg-rose-300/10" },
  { level: 10, icon: "🕋", title: "Sadiq", titleAr: "صادق", minXp: 14000, color: "text-cyan-300", bg: "bg-cyan-300/10" },
  { level: 11, icon: "🌟", title: "Muqarrab", titleAr: "مقرّب", minXp: 21000, color: "text-orange-300", bg: "bg-orange-300/10" },
  { level: 12, icon: "👑", title: "Guardian", titleAr: "ولي", minXp: 30000, color: "text-yellow-300", bg: "bg-yellow-300/10" },
];
...
export const XP_ACTIONS = {
  ...
  thematic_search: { xp: 4, label: "Searched by theme" },
  hadith_explore: { xp: 4, label: "Explored hadith library" },
  audio_page_complete: { xp: 8, label: "Completed page recitation" },
  /** Gentle reward for staying with the app (milestone nudges). */
  presence_milestone: { xp: 1, label: "Time with the Quran" },
...
export const BADGES = {
  ...
  search_5: { id: "search_5", icon: "🔎", title: "Navigator", desc: "Completed 5 thematic searches", xp: 20 },
  audio_page_3: { id: "audio_page_3", icon: "🎧", title: "Attentive Listener", desc: "Completed 3 page recitations", xp: 25 },
  night_owl: {
    id: "night_owl",
    icon: "🌙",
...
/**
 * Merges raw persisted state with defaults and applies daily rollover rules.
 * Safe to run on the server (API) or client.
 */
export function normalizeGamificationState(raw) {
  const base = createDefaultGamificationState();
  const state =
    typeof raw === "object" && raw !== null && !Array.isArray(raw) ? { ...base, ...raw } : { ...base };
  ...
  state.surahs_read = state.surahs_read ?? [];
  state.total_minutes_spent = state.total_minutes_spent ?? 0;
  return state;
}

export function loadState(userId) {
  if (typeof window === "undefined" || !userId) return createDefaultGamificationState();
  try {
    const raw = localStorage.getItem(storageKey(userId));
    const parsed = raw ? JSON.parse(raw) : createDefaultGamificationState();
...
function checkBadges(state) {
  ...
  if ((totalActions.thematic_search ?? 0) >= 5) earn("search_5");
  if ((totalActions.audio_page_complete ?? 0) >= 3) earn("audio_page_3");
...
/**
 * Applies streak XP/badge tiers without double-counting. `awardXP` grants XP + actionLog entries; badges are mirrored into `badges[]` for Profile UI (not wired from `XP_ACTIONS` alone).
 */
export function awardStreakBadge(state, streakDays) {
  const earned = [];
  /** @type {Array<"streak_3"|"streak_7"|"streak_30">} */
  const tier =
    streakDays >= 30 ? ["streak_3", "streak_7", "streak_30"] : streakDays >= 7 ? ["streak_3", "streak_7"] : streakDays >= 3 ? ["streak_3"] : [];
  for (const actionKey of tier) {
    if (state.badges.includes(actionKey)) continue;
    awardXP(state, actionKey);
    state.badges.push(actionKey);
    state.badgeLog = state.badgeLog ?? [];
    state.badgeLog.push({ id: actionKey, earnedAt: Date.now() });
    const badge = BADGES[actionKey];
