...
export function aggregateGoalSignals(events) {
  ...
  for (const e of Array.isArray(events) ? events : []) {
    ...
    const iso =
      typeof e?.occurred_at === "string" ? e.occurred_at : typeof e?.created_at === "string" ? e.created_at : "";
...
export function computeDerivedProgress(goal, agg) {
  ...
  switch (type) {
    case "complete_quran": {
      ...
      const checkpoints = Math.max(
        juzCheckpointFromPages(agg.audioPages),
        Math.min(total, Math.floor(agg.readVerseTicks / 40)),
      );
...
export function normalizeApiGoal(raw, templates) {
  ...
  const templateId =
    typeof raw.type === "string" ? raw.type : typeof meta.templateId === "string" ? meta.templateId : "custom";
