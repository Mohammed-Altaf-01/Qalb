...
function tearDownPlayers() {
  if (madinahStaggerTimer != null) {
    clearTimeout(madinahStaggerTimer);
    madinahStaggerTimer = null;
  }
...
/**
 * Only the active slot fetches HLS segments; inactive uses `stopLoad`.
 */
function syncInactiveHlsLoad() {
  const makkahActive = activeSlot === "makkah";
  if (makkahHls) {
    try {
      if (makkahActive) {
        makkahHls.startLoad(-1);
...
export function getLiveDualLoadPolicy(slot) {
  const makkahActive = slot !== "madinah";
  return {
    makkah: makkahActive ? "load" : "stop",
    madinah: makkahActive ? "stop" : "load",
  };
}
...
function startMadinahHls(madinahUrl) {
  if (!madinahVideo || madinahHls || !madinahUrl) return;
...
export async function ensureLiveDualPrewarm(channels) {
  ...
  let madinahStarted = false;
  const tryStartMadinah = () => {
    if (madinahStarted) return;
    madinahStarted = true;
    startMadinahHls(madinahUrl);
  };

  makkahHls = attachHlsToVideo(makkahVideo, makkahUrl, (hls) => {
  ...
  if (makkahHls) {
    makkahHls.on("hlsFragBuffered", () => {
      tryStartMadinah();
    });
    madinahStaggerTimer = setTimeout(tryStartMadinah, 4000);
  } else if (makkahVideo) {
