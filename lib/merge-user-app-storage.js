 ...
 */
import { normalizeListenHistoryPayload } from "@/lib/listen-history.js";
 ...
 */
export function mergePreferencesPayload(
  remote,
  localTheme,
  localScale,
  localReciter,
  ...
  const liveVideoFit =
    r.liveVideoFit === "contain" || r.liveVideoFit === "cover"
      ? r.liveVideoFit
      : localLiveVideoFit === "contain"
        ? "contain"
        : "cover";
  const hifzPrefs =
    r.hifzPrefs && typeof r.hifzPrefs === "object" && !Array.isArray(r.hifzPrefs)
      ? r.hifzPrefs
      : localHifzPrefs && typeof localHifzPrefs === "object" && !Array.isArray(localHifzPrefs)
        ? localHifzPrefs
        : null;
  return {
    theme,
...
/**
 * Merge listen_history payloads (remote + local). Newer updatedAt wins per reciter+surah; cap 5 per reciter.
 * @param {object | null} remote
 * @param {object | null} local
 */
export function mergeListenHistoryPayload(remote, local) {
  const r = remote && typeof remote === "object" && !Array.isArray(remote) ? remote : {};
  const l = local && typeof local === "object" && !Array.isArray(local) ? local : {};
  const rNorm = normalizeListenHistoryPayload(r);
  const lNorm = normalizeListenHistoryPayload(l);
  const byKey = new Map();
  for (const e of [...rNorm.entries, ...lNorm.entries]) {
    const key = `${e.reciterId}:${e.surahId}`;
    const prev = byKey.get(key);
    if (!prev || (e.updatedAt ?? 0) >= (prev.updatedAt ?? 0)) byKey.set(key, e);
  }
  return normalizeListenHistoryPayload({ entries: [...byKey.values()] });
}
...
/**
 * Merge reading goals lists by `id`; newer `updatedAt` / `createdAt` wins.
 * @param {unknown} remoteList
 * @param {unknown} localList
 * @param {number} [maxItems]
 */
export function mergeReadingGoals(remoteList, localList, maxItems = 50) {
  ...
  for (const g of [...r, ...l]) {
    ...
    if (!id) continue;
    const prev = byId.get(id);
    if (!prev || goalSortKey(g) >= goalSortKey(prev)) byId.set(id, g);
  }
  return [...byId.values()].sort((a, b) => goalSortKey(b) - goalSortKey(a)).slice(0, maxItems);
}

/**
 * Merge discover history entries (dedupe by snippet + timestamp).
 * @param {unknown} remoteList
 * @param {unknown} localList
 * @param {number} [maxItems]
 */
export function mergeDiscoverHistory(remoteList, localList, maxItems = 25) {
  const remote = Array.isArray(remoteList) ? remoteList : [];
  const local = Array.isArray(localList) ? localList : [];
  const seen = new Set();
  const out = [];
  for (const item of [...remote, ...local]) {
    ...
    const key = `${String(item.situationSnippet ?? "")}|${item.at ?? 0}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(item);
  }
  return out.sort((a, b) => (Number(b?.at) || 0) - (Number(a?.at) || 0)).slice(0, maxItems);
}
