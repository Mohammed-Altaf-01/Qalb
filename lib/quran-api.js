...
class RequestBuilder {
  ...
  async fetch() {
    ...
    const res = await fetch(url, {
      ...
      // Cache for 5 minutes on the server — Quran content rarely changes
      next: { revalidate: 300 },
    });
...
class QuranRepository {
  ...
  static async getChapters(language = "en") {
    return new RequestBuilder("/content/api/v4/chapters").withParam("language", language).fetch();
  }
  ...
  static async getChapter(chapterId, language = "en") {
    return new RequestBuilder(`/content/api/v4/chapters/${chapterId}`).withParam("language", language).fetch();
  }
   ...
   * @param {number} chapterId - Chapter number (1–114)
   * @param {object} [opts]
   * @param {number} [opts.translationId=131] - Translation resource ID (131 = Sahih International)
  ...
  static async getVersesByChapter(chapterId, opts = {}) {
    const { translationId = 131, language = "en", perPage = 10, page = 1 } = opts;
    return new RequestBuilder(`/content/api/v4/verses/by_chapter/${chapterId}`)
      ...
      .withParam("audio", 7) // Mishary Rashid Alafasy — universally recognized reciter
      .fetch();
  }
  ...
  static async getReciters(language = "en") {
    return new RequestBuilder("/content/api/v4/resources/recitations").withParam("language", language).fetch();
  }
   ...
   * @returns {Promise<{audio_files: Array}>}
   */
  static async getVerseAudio(verseKey, recitationId = 7) {
    return new RequestBuilder(`/content/api/v4/recitations/${recitationId}/by_ayah/${verseKey}`).fetch();
  }
  ...
  static async getTafsirList(language = "en") {
    return new RequestBuilder("/content/api/v4/resources/tafsirs").withParam("language", language).fetch();
  }
  ...
  static async getTafsirByChapter(chapterId, tafsirId = 169) {
    return new RequestBuilder(`/content/api/v4/tafsirs/${tafsirId}/by_chapter/${chapterId}`).fetch();
  }
  ...
  static async getTafsirByVerse(verseKey, tafsirId = 169) {
    return new RequestBuilder(`/content/api/v4/tafsirs/${tafsirId}/by_ayah/${verseKey}`).fetch();
  }
  ...
  static async getTranslationList(language = "en") {
    return new RequestBuilder("/content/api/v4/resources/translations").withParam("language", language).fetch();
  }
  ...
  static async getJuzList() {
    return new RequestBuilder("/content/api/v4/juzs").fetch();
  }
}
