import { quranApiLog } from "@/lib/logger";
...
class RequestBuilder {
  ...
  async fetch() {
    ...
    quranApiLog.info("content_fetch", {
      path: this.path,
      status: res.status,
      durationMs,
      cacheRevalidate: this._revalidate,
    });

    if (!res.ok) {
      quranApiLog.warn("content_fetch_error", {
        path: this.path,
        status: res.status,
...
class QuranRepository {
  ...
  static async searchVerses(query, opts = {}) {
    ...
    const safeSize = Number.isFinite(size) ? Math.min(30, Math.max(1, Number(size))) : 10;
    const safePage = Number.isFinite(page) ? Math.max(0, Number(page)) : 0;
    return new RequestBuilder("/content/api/v4/search")
      .withParam("q", query)
      .withParam("size", safeSize)
      .withParam("page", safePage)
      .withParam("language", language)
  ...
  /**
   * Hizb subdivisions (paired quarter-juz markers in some prints).
   * @param {string} [language='en']
   */
