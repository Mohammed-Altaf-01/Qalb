...
import { afterQuranPlaybackEnd, beforeQuranPlayback } from "@/lib/audio-focus";
...
export async function startExternalQuranAudio({
  ...
  const seekTo = Number.isFinite(startAtSec) && startAtSec > 0 ? startAtSec : 0;
  if (seekTo > 0) {
    const onMeta = () => {
      a.removeEventListener("loadedmetadata", onMeta);
      if (myJob !== jobId) return;
      const max = Number.isFinite(a.duration) && a.duration > 0 ? a.duration : 0;
      a.currentTime = max > 0 ? Math.min(seekTo, max) : seekTo;
