...
function pauseLiveDualForAudio() {
  ...
  void import("@/lib/live-dual-prewarm")
    .then((m) => m.pauseLiveDualPrewarm())
    .catch(() => {});
}
...
function resumeLiveDualAfterAudio() {
  if (typeof window === "undefined") return;
  void import("@/lib/live-dual-prewarm")
    .then((m) => m.resumeLiveDualPrewarm())
    .catch(() => {});
}
...
export async function startQuranAudio({ mode, reciterId, reciterName, surahId, startVerse = 1, surahName, maxVerseNum = null }) {
  const a = ensureAudio();
