...
import { GAMIFICATION_MINUTE_TICK_MS, GAMIFICATION_REMOTE_SYNC_DEBOUNCE_MS } from "@/lib/constants/gamification";
...
export function useGamification() {
  ...
  useEffect(() => {
    ...
    const commitPendingMinutes = () => {
      ...
      let streakBadges = [];
      try {
        const ttRaw = localStorage.getItem(LS_TIME_TRACKING);
        const tt = ttRaw ? JSON.parse(ttRaw) : {};
        const byDay = tt?.byDay && typeof tt.byDay === "object" ? tt.byDay : {};
        const streak = computeDayStreak(byDay, toLocalDayKey());
        streakBadges = awardStreakBadge(draft, streak);
      } catch {
        /* ignore */
      }
      if (result.xpGained > 0 || (result.newDeeds?.length ?? 0) > 0 || (streakBadges?.length ?? 0) > 0) {
      ...
      if ((streakBadges?.length ?? 0) > 0) {
        streakBadges.forEach((b) => b && showBadgeToast(b));
      }
    };
