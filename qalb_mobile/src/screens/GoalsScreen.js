      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {goals.length > 0 && (
          <>
            ...
            {goals.map((goal) => (
              <GoalCard key={goal.id} goal={goal} onDelete={deleteGoal} onToggleDay={toggleDay} />
            ))}
          </>
        )}

        {/* Goal templates */}
        <Text style={styles.sectionLabel}>{goals.length === 0 ? "Start with a goal" : "Add another"}</Text>
        ...
        <TouchableOpacity style={styles.customBtn} onPress={() => setShowModal(true)}>
